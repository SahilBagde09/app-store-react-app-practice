...
class AppStore extends Component {
  ...
  onChangeSearch = event => {
    ...
    // const firstLetter = searchItem.slice(0, 1).toUpperCase()
    // const capitalizedSearch =
    //   firstLetter + searchItem.slice(1, searchItem.length)
    this.setState({changeSearch: searchItem})
  }
  ...
  render() {
    const {changeSearch, currentTab} = this.state
    const updatedList = appsList.filter(
      eachApp =>
        eachApp.category === currentTab &&
        eachApp.appName.includes(changeSearch),
    )
    ...
    return (
      <div className="bg-container">
        ...
        <ul className="tabIItem">
          {tabsList.map(eachTabItem => (
            <TabItem
              tabsDetails={eachTabItem}
